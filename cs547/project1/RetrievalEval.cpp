...
double computeRawTFIDFWeight(int docID,
		     ...
		     Index *ind)
{
  return docTermFreq*log(ind->docCount()/(ind->termCount(termID)+1))*qryTermWeight;
}
...
double computeOkapiWeight(int docID,
		     ...
		     Index *ind)
{
  double temp = docTermFreq/(docTermFreq+0.5+1.5*ind->docLength(docID)/ind->docLengthAvg());
  temp*=log((ind->docCount()-ind->termCount(termID) + 0.5)/(ind->termCount(termID) + 0.5));
  temp*=((8+qryTermWeight)/(7+qryTermWeight));
  return temp;
}
...
double computeCustomWeight(int docID,
		     ...
		     Index *ind)
{
  double temp = docTermFreq * 0.5* (ind->docLengthAvg()/ind->docLength(docID))*log(ind->docCount()/(ind->termCount(termID)+1))*qryTermWeight;
  return temp;
}
